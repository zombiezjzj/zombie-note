...
```ts
let sym: symbol = Symbol('zombie')
sym = Symbol('zombie') // OK
sym = 'zombie' // Error
```
...
// 任意类型
const anyArray: Array<any> = ['zombie', 18, {}]
```
...
```ts
const tuple: [string, number] = ['zombie', 18]
...
```ts
type Name = 'zombie' | 'zombie1996' | 'zombie'
```
